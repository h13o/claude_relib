# rosenka — 最寄駅の土地取引価格を「表＋散布図」で見る

駅名や住所を入れると、その**最寄駅の宅地（土地）取引価格**を、**過去3年・新しい順の表**と
**2つの散布図**（① 取引年次×価格 ／ ② 駅徒歩分数×価格）で表示する単一HTMLです。
データは国土交通省 **[不動産情報ライブラリ](https://www.reinfolib.mlit.go.jp/)** の取引価格API（XPT001）。
地図ライブラリも外部チャートライブラリも使わない、依存ゼロの1ファイル構成です。
...
- 駅名・住所で検索 → 周辺の宅地（土地）取引を取得し、**最寄駅ごとにまとめて**表示（住所/駅検索は地理院ジオコーダ＝無料・キー不要）
- ヘッダの**駅セレクタ**で、見つかった駅を切り替え（既定は検索地点に最も近い駅）
- **表**：過去3年・新しい順。総額／㎡単価／面積／間口／前面道路／用途地域／建ぺい・容積／最寄駅／徒歩(分)／市区町村・地区
- **散布図①** 取引年次 × 価格、**散布図②** 駅徒歩(分) × 価格（点にカーソルを合わせると明細をツールチップ表示）
- 縦軸は**総額／㎡単価**をセレクタで切替
- `?q=駅名` または `?lat=&lng=` のURLで**ダイレクトに開く**（ホーム画面ブックマーク向き）

> 「最寄駅」「駅徒歩分数」はAPIレスポンスのフィールドから取得します。レスポンスに駅徒歩分数が含まれない場合、散布図②と表の「徒歩(分)」は「—」表示になります。
...
| ファイル | 内容 |
|----------|------|
| `index.html` | アプリ本体（表＋SVG散布図／不動産情報ライブラリXPT001）。設定は冒頭`CONFIG`のみ・外部ライブラリ依存なし |
